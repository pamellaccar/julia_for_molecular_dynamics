# uma possibilidade para isso seria gerar automaticamente pontos iniciais mais próximos.
# Simplex minimization
function simplex(f, x0, niter, scale_factor)
    # Initial point: x0 contains 3 vectors of dimension 3
    x0 = [
        [rand() * scale_factor + x0[1][1], rand() * scale_factor + x0[1][2], rand() * scale_factor + x0[1][3]],
        [rand() * scale_factor + x0[2][1], rand() * scale_factor + x0[2][2], rand() * scale_factor + x0[2][3]],
        [rand() * scale_factor + x0[3][1], rand() * scale_factor + x0[3][2], rand() * scale_factor + x0[3][3]]
    ]
    x = copy(x0)
